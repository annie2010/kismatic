...
func NewCmdSeedRegistry(stdout, stderr io.Writer) *cobra.Command {
	...
	cmd.Flags().StringVar(&options.registryServer, "server", "", "set to the location of the registry server (e.g. localhost:5000)")
...
func doListImages(out io.Writer, options seedRegistryOptions, imageManifestFile string) error {
	...
	varsBytes, err := ioutil.ReadFile(imageManifestFile)
	if err != nil {
		return fmt.Errorf("Error reading the list of images: %v", err)
	}
	vars := imageManifest{}
	if err = yaml.Unmarshal(varsBytes, &vars); err != nil {
		return fmt.Errorf("Error unmarshalling the list of images: %v", err)
	}
	for _, img := range vars.OfficialImages {
		fmt.Fprintf(out, "%s\n", img)
	}
	return nil
}
...
func doSeedRegistry(stdout, stderr io.Writer, options seedRegistryOptions, imageManifestFile string) error {
	...
	if server == "" {
		...
		if !planner.PlanExists() {
			...
			fmt.Fprintln(stdout, "Run \"kismatic install plan\" to generate it")
